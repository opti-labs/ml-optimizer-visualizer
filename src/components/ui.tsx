...
export function ParamControl({
  ...
  const accentMap: Record<string, string> = {
    indigo: 'accent-indigo-500', emerald: 'accent-emerald-500', orange: 'accent-orange-500',
  };
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between mb-1 gap-2">
        ...
        <input
          ...
          className="w-20 rounded-md border border-slate-600 bg-slate-700/60 px-2 py-0.5 text-xs text-right font-mono text-indigo-300 focus:border-indigo-500 focus:outline-none"
        />
      </div>
      <input
        type="range" min={min} max={max} step={step} value={value}
        onChange={e => onChange(Number(e.target.value))}
        className={`w-full h-1.5 appearance-none rounded-full bg-slate-600 cursor-pointer ${accentMap[accent] || accentMap.indigo}`}
      />
    </div>
  );
}
...
export function Button({
  ...
  const styles: Record<string, string> = {
    primary: 'bg-indigo-600 hover:bg-indigo-500 text-white border-indigo-500 shadow-lg shadow-indigo-900/30',
    ghost: 'bg-slate-700/60 hover:bg-slate-600 text-slate-200 border-slate-600',
    amber: 'bg-amber-600 hover:bg-amber-500 text-white border-amber-500',
    danger: 'bg-slate-800 hover:bg-slate-700 text-slate-400 border-slate-600',
  };
    ...
    <div className="flex items-center gap-4 mb-6">
      {onBack && (
        <button onClick={onBack}
          className="rounded-lg border border-slate-600 bg-slate-800/60 px-3 py-1.5 text-xs text-slate-300 hover:border-slate-500 transition-colors">
          ← 戻る
        </button>
      )}
      ...
      <div className="ml-auto flex items-center gap-1.5">
        {steps.map((s, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <span className={`flex items-center gap-1.5 text-xs ${i === current ? 'text-indigo-300' : 'text-slate-600'}`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold border ${
                i === current ? 'bg-indigo-600 border-indigo-500 text-white'
                  : i < current ? 'bg-slate-700 border-slate-600 text-slate-400' : 'border-slate-700 text-slate-600'
              }`}>{i + 1}</span>
